...
async function getData(user_id){
    ...
    const { data:user } = await axios("https://jsonplaceholder.typicode.com/users/"+ user_id); // bu dondu cunku data! 

    const {data:post} = await axios("https://jsonplaceholder.typicode.com/posts/"+ user_id); //undefined dedi
    console.log(user);
    console.log(post);
